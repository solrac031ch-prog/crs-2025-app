...
const jsPath = 'gestion-panel-final.js';
const cssPath = 'gestion-panel-final.css';
...
if (fnStart < 0 || nextFn < 0) throw new Error('No se pudo delimitar addStyle().');

const block = source.slice(fnStart, nextFn);
const cssMatch = block.match(/style\.textContent\s*=\s*`([\s\S]*?)`;\s*\n/);
if (!cssMatch) throw new Error('No se encontró el CSS inyectado de Gestión.');
...
if (!css.includes('.gf-shell') || !css.includes('.gf-paper-layout') || !css.includes('@media')) {
...
console.log('CSS de Gestión extraído sin modificar sus reglas.');
