(() => {
  ...
  function resetGoogleSelector() {
    ...
    if (target) {
      target.innerHTML = "";
      delete target.dataset.googleReady;
    }
    if (status) {
      status.textContent = "Selector reiniciado. Presiona el botón de Google. Si la cuenta no aparece, primero ábrela en Chrome o usa una ventana incógnita.";
    }
    setTimeout(initGoogleButton, 80);
  }

  function loginHelpHtml() {
    return `<div data-google-login-help class="stable-note" style="margin-top:10px;line-height:1.35"><strong>¿No aparece el correo?</strong><br>Agregar un usuario en Jefatura solo lo autoriza. Google mostrará las cuentas abiertas en este navegador. Para otro jefe: abrir esa cuenta en Chrome o usar ventana incógnita y luego entrar a la web.<br><button class="document-button" type="button" data-google-reset-selector style="margin-top:8px">Reiniciar selector Google</button></div>`;
  }
  ...
  async function verifyWithAppsScript(email, credentialPayload = {}) {
    ...
    if (!scriptUrl) {
      return {
        ...
        role: adminEmails.has(email) ? "admin" : "equipo",
  ...
  function initGoogleButton() {
    ...
    if (!config.googleClientId) {
      target.innerHTML = `<div class="stable-note"><strong>Google Auth preparado.</strong><br>Falta pegar el Google Client ID en <code>google-auth-config.js</code>.</div>${loginHelpHtml()}`;
      return;
    }
  ...
  function injectAuthBox() {
    ...
    box.innerHTML = session
      ? `<strong>Cuenta Google conectada</strong><span>${esc(session.email)} · ${esc(session.role)}</span><button class="document-button" type="button" data-google-logout>Cerrar sesión Google</button>`
      : `<strong>Ingreso con Google</strong><span>Autorizado por backend Google.</span><div id="googleSignInButton"></div><div data-google-auth-status class="stable-note"></div>`;
    panel.prepend(box);
    setTimeout(initGoogleButton, 80);
  }
  ...
  document.addEventListener("click", (event) => {
    ...
    if (event.target.closest("[data-google-retry]")) {
      const target = $("#googleSignInButton");
      if (target) delete target.dataset.googleReady;
      initGoogleButton();
    }
    if (event.target.closest("[data-google-reset-selector]")) resetGoogleSelector();
  }, true);
